...
const WelcomeCard = require('./adaptiveCards/welcome-card.json');
const AttendanceSummaryCard = require('./adaptiveCards/attendance-summary-card.json');
const DisabledChatCard = require('./adaptiveCards/chatdisabled-card.json');
...
export class TeamsBot extends TeamsActivityHandler {
  ...
  constructor() {
    ...
    this.onTeamsMeetingEndEvent(async (meeting, context, next) => {
      try {
        var meetingDetails : any = {};
        meetingDetails  = await TeamsInfo.getMeetingInfo(context);        
        var result = await GraphHelper.getNewestMeetingAttendanceRecordsAsync(meetingDetails.details.msGraphResourceId, meetingDetails.organizer.aadObjectId);
        if (result != null) {
          const card = Utils.renderAdaptiveCard(AttendanceSummaryCard, result);
          await context.sendActivity({ attachments: [card] });
          await context.sendActivity({ attachments: [CardFactory.adaptiveCard(DisabledChatCard)] });
        }        
      }
      catch (ex) {
